...
//        contentScrollView.delegate = self;
    ...
    // 第四页
    {
        ...
        // 按钮
        [forthBtn setImage:[UIImage imageNamed:@"4-btn"] forState:UIControlStateNormal];
        forthBtn.bounds = CGRectMake(0, 0, 150, 40);
        forthBtn.center = CGPointMake(ScreenWidth/2, 465);
        forthBtn.transform = CGAffineTransformMakeScale(0, 0);
    ...
    [self bringSubviewToFront:gestureScrollView];
    [self bringSubviewToFront:forthBtn];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 相框
    {
        ...
        if (offset < ScreenWidth*2) {
            ...
            CGFloat height = calculate(280, 224,  ScreenWidth*2.15,  ScreenWidth*3, offset);
            CGFloat width = calculate(196, 145, ScreenWidth*2.15, ScreenWidth*3, offset);
            
            CGRect bound = CGRectMake(0, 0, width, height);
            contentBackgroundView.bounds = bound;
...
//        320 640
        if (offset < ScreenWidth*2) {
            ...
            CGFloat dis = ScreenWidth * 0.24;
            CGFloat delta = ScreenWidth * 0.1;
            
            // 相框旋转
            CGFloat angle = calculate(0, -M_PI_4/3, ScreenWidth * 2, ScreenWidth * 3, offset);
    ...
    // 第四页
    {
        ...
        // 开启完美搭配按钮
        CGFloat scale;
        if (offset < ScreenWidth*2.6) {
            scale = calculate(0, 1.2, ScreenWidth*2.3, ScreenWidth*2.6, offset);
            forthBtn.transform = CGAffineTransformMakeScale(scale, scale);
        }else{
            scale = calculate(1.2, 1, ScreenWidth*2.6, ScreenWidth*3, offset);
            forthBtn.transform = CGAffineTransformMakeScale(scale, scale);
        }
        
        // 左右两张图片
        forthLeft.alpha = calculate(0, 0.8, ScreenWidth*2.3, ScreenWidth*2.6, offset);
        forthRight.alpha = calculate(0, 0.8, ScreenWidth*2.3, ScreenWidth*2.6, offset);
        
        forthLeft.centerX = calculate(centerX, 0-10, ScreenWidth*2.5, ScreenWidth*3, offset);
        forthRight.centerX = calculate(centerX, ScreenWidth+10, ScreenWidth*2.5, ScreenWidth*3, offset);
